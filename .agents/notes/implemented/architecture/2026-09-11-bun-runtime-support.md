...
Starting the harness under Bun (1.1.0+) failed in five places:
...
5. `NotImplementedError: worker_threads.Worker.performance is not yet implemented in Bun` — the worker compute budget's utilization poll threw from inside a timer callback, so the throw killed the host process instead of failing the run.
...
6. **Busy-time capability probe** — `code-runtime-worker-thread` calls `worker.performance.eventLoopUtilization()` once per service before polling it, and skips the poll entirely when the runtime cannot report worker busy time. A runtime can expose that method and still not implement it: Bun's stub returns a constant zero sample and reports `ERR_NOT_IMPLEMENTED`. The skipped poll is announced once on stderr together with both configured budgets, because a `computeMs` that cannot expire a run must never be silent; `maxWallMs` remains the run's ceiling.
...
- Under Bun no run is bounded by busy time — the runtime cannot report it — so `maxWallMs` is the only ceiling and the harness says so at the first run. A deployment that needs busy-time containment runs Node.
